...
class GameCatalogueTests(unittest.TestCase):
    def test_plugin_version_is_visible_and_matches_manifest(self):
        ...
        self.assertIn('text: "Yoink version " + root.pluginVersion', bar_widget)
        ...
        self.assertIn('text: "created by Demmel"', bar_widget)
        self.assertNotIn('text: "Save To"', bar_widget)
        self.assertNotIn('id: destination', bar_widget)
        self.assertNotIn('id: videoDestination', bar_widget)
        self.assertIn('text: "TV"', bar_widget)
        self.assertIn('placeholderText: "Search movie or show…"', bar_widget)
        self.assertIn('text: "TV Shows"', bar_widget)
        self.assertIn('text: "← Back to catalogue"', bar_widget)
        self.assertGreaterEqual(bar_widget.count('type: root.movieBrowseType'), 2)
        self.assertIn('action in (\'movie-trending\', \'movie-search\', \'movie-detail\', \'movie-sources\')', (project / 'backend.py').read_text())
        self.assertIn('trendingTv', (project / 'movies.py').read_text())
        self.assertIn('searchMulti', (project / 'movies.py').read_text())
